...
export async function assembleAvatar(state: AvatarProfile): Promise<THREE.Group> {
  ...
  const isFallback = !!(body as any).userData?.isFallback;
  if (!isFallback) {
  ...
  for (const [slot, styledNft] of Object.entries(state.wearables)) {
    ...
    const meshKey = getMeshPath('wearables', (styledNft as StyledNFTRef).styleKey);
    ...
    if (meshKey) {
      ...
      console.warn(`Wearable mesh not found for slot: ${slot} with styleKey: ${(styledNft as StyledNFTRef).styleKey}`);
    }
  }
  ...
  if (state.props.handRight) {
    ...
    const resolved = await resolveNFT(styledNft);
    const meshKey = getMeshPath('props', (styledNft as StyledNFTRef).styleKey);
    
    if (meshKey) {
      const wearable = await attachWearable(body, 'hand.right', meshKey, resolved.imageUrl);
      (wearable as any).userData.type = "prop";
  ...
  if (state.props.floating && state.props.floating.length > 0) {
    for (const styledNft of state.props.floating) {
      ...
      if (meshKey) {
        ...
        (prop as any).userData.type = "prop";
      ...
      if (petPath) {
        ...
        petMesh.traverse(async (child) => {
          if (child instanceof THREE.Mesh && (child as any).material) {
            ((child as any).material as THREE.Material).clone();
            const media = await resolveNFTMedia(resolved.imageUrl);
            if (media.type === "texture") {
              (((child as any).material as THREE.MeshStandardMaterial).map = media.texture);
            }
          }
        });
        
        group.add(petMesh);
        (petMesh as any).userData.type = "pet"; // Tag for ZoneManager
        
        // Store pet instance data for external update loop (PetSystem)
        (group as any).userData.petInstance = {
          model: petMesh,
          followDistance: 2.0,
        };
      }
  ...
  if (state.aura) {
    ...
    group.add(aura);
    (aura as any).userData.type = "effect"; // Tag for ZoneManager
  }
