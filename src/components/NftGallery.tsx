...
const ceilingFragmentShader = `
  ...
  void main() {
    ...
    // Star field: dispersed random stars only (no grid lines)

    float starField = 0.0;

    // Base tiny stars (more numerous)
    float r1 = rand(vUv * 120.0 + time * 0.05);
    float tiny = pow(r1, 35.0);
    starField += tiny * 0.9;

    // Some brighter, larger-feeling stars
    float r2 = rand(vUv * 80.0 - time * 0.03);
    float big = pow(r2, 12.0);
    starField += big * 1.8;
