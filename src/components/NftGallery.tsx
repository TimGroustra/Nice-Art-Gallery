...
const NftGallery: React.FC<NftGalleryProps> = ({ setInstructionsVisible }) => {
  ...
  useEffect(() => {
    ...
    // Define innermost wall geometry variables once
    ...
    // Re-use definitions from above for construction, but define them here for scope clarity
    // We only need the calculated offsets here, the geometry variables are already defined above.
    // NOTE: The variables innermostDoorwayWidth, innermostWallSegmentWidth, innermostSegmentOffset 
    // were defined in the construction block above (lines 470-472) and then redefined 
    // in the panelConfigs loop (lines 662-664). I am consolidating them to the construction block.
    
    // Since the construction block already defines them, I will remove the redundant definitions 
    // from the panelConfigs loop below and ensure the construction block definitions are correct.
    
    // --- Innermost Room Construction Variables (already defined above) ---
    // const innermostDoorwayWidth = 8;
    // const innermostWallSegmentWidth = (innermostRoomSize - innermostDoorwayWidth) / 2;
    // const innermostWallSegmentGeometry = new THREE.PlaneGeometry(innermostWallSegmentWidth, wallHeight);
    // const innermostSegmentOffset = innermostDoorwayWidth / 2 + innermostWallSegmentWidth / 2;
    
    // The geometry definition was missing, adding it here:
    // Re-defining the innermost variables here to ensure they are available for the geometry creation
    const innermostDoorwayWidth = 8;
    const innermostWallSegmentWidth = (innermostRoomSize - innermostDoorwayWidth) / 2;
    const innermostWallSegmentGeometry = new THREE.PlaneGeometry(innermostWallSegmentWidth, wallHeight);
    const innermostSegmentOffset = innermostDoorwayWidth / 2 + innermostWallSegmentWidth / 2;
