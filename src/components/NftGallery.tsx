...
const NftGallery: React.FC<NftGalleryProps> = ({ setInstructionsVisible }) => {
  ...
  const applyFallbackContent = useCallback((panel: Panel, collectionName: string, error: string) => {
    ...
    // Define descriptionText here to resolve TS2552
    const descriptionText = `Error loading token ${GALLERY_PANEL_CONFIG[panel.wallName].tokenIds[GALLERY_PANEL_CONFIG[panel.wallName].currentIndex]}. Check console for details.`;
    const { texture: descriptionTexture, totalHeight } = createTextTexture(descriptionText, TEXT_PANEL_WIDTH, DESCRIPTION_PANEL_HEIGHT, 30, 'red', { wordWrap: true });
