...
const ceilingFragmentShader = `
  ...
  void main() {
    ...
    // Star field: ONLY dispersed random stars (no grid, no lines)

    float starField = 0.0;

    // One random sample for base tiny stars
    float r1 = rand(vUv * 120.0 + time * 0.05);
    float tiny = pow(r1, 35.0);       // adjust exponent for density
    starField += tiny * 0.9;

    // A second sample for a few brighter “larger” stars
    float r2 = rand(vUv * 80.0 - time * 0.03);
    float big = pow(r2, 12.0);        // lower exponent -> more/larger stars
    starField += big * 1.8;

    // Gentle twinkle using time
    float twinkle = 0.65 + 0.35 * sin(time * 0.9 + r2 * 20.0);
    starField *= twinkle;
