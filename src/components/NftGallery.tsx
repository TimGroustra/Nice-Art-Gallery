...
const NftGallery: React.FC<NftGalleryProps> = ({ setInstructionsVisible }) => {
  ...
  useEffect(() => {
    ...
    // Furniture loading: Sofa and Tables
    ...
    gltfLoader.load('/assets/models/sofa.glb', (gltf) => {
      ...
      if (extractedSofa) {
        ...
        const sofaPositions = [
          { x: 0, z: 4.5 }, { x: 0, z: -4.5 }, { x: 4.5, z: 0 }, { x: -4.5, z: 0 },
        ];
    ...
    gltfLoader.load('/assets/models/Wood_Table.glb', (gltf) => {
      ...
      // Auto-scale the table to ~1.8 meters wide
      ...
      // Re-center Y position
      const adjustedBox = new THREE.Box3().setFromObject(tableModel);
      const bottomY = adjustedBox.min.y;

      // Position tables in front of each couch
      const tablePositions = [
        { x: 0, z: 1.8 }, { x: 0, z: -1.8 }, { x: 1.8, z: 0 }, { x: -1.8, z: 0 },
      ];

      tablePositions.forEach(pos => {
        const table = tableModel.clone();
        table.position.set(pos.x, PLATFORM_Y + WALL_THICKNESS / 2 - bottomY, pos.z);
        // Rotate tables to align with the seating (though center-facing tables might not need specific rotation if symmetrical)
