...
interface Panel {
  ...
  // No wallTitleMesh – text on walls removed
...
const NftGallery: React.FC<{ setInstructionsVisible: (v: boolean) => void }> = ({ setInstructionsVisible }) => {
  ...
  useEffect(() => {
    ...
    // ----- OCTAGON WALLS (rotated 90° so ends meet) -----
    ...
    for (let i = 0; i < 8; i++) {
      ...
      // Rotate inward and add 90° to connect ends
      const wallAngle = Math.atan2(z2 - z1, x2 - x1) + Math.PI / 2;
      wallMesh.rotation.y = -(wallAngle) + Math.PI / 2;
    ...
    // Compute panel positions/orientations around octagon
    ...
    const animate = () => {
      ...
      if (controls.isLocked) {
        ...
        if (hits.length && hits[0].distance < 5) {
          ...
          const panel = panelsRef.current.find(p =>
            p.mesh === hit || p.prevArrow === hit || p.nextArrow === hit || p.descriptionMesh === hit
          );
          if (panel) {
