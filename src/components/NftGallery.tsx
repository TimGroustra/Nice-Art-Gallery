...
const NftGallery: React.FC<NftGalleryProps> = ({ setInstructionsVisible }) => {
  ...
  useEffect(() => {
    ...
    // --- SPIRAL STAIRCASE IN THE 10x10 AREA ---
    const createSpiralStaircase = () => {
        const STAIRCASE_CENTER_X = 0;
        const STAIRCASE_CENTER_Z = 0;
        const STAIRCASE_BASE_Y = 0;
        const STAIRCASE_HEIGHT = 4; // Height to reach second floor
        const STAIRCASE_RADIUS = 1.5; // Radius of the staircase
        const CENTRAL_COLUMN_RADIUS = 0.3; // Radius of the central column
        const STEP_COUNT = 16; // Number of steps in the spiral
        const STEP_HEIGHT = STAIRCASE_HEIGHT / STEP_COUNT;
        const STEP_ANGLE_INCREMENT = (2 * Math.PI) / STEP_COUNT;
        
        // Central column
        const columnGeometry = new THREE.CylinderGeometry(CENTRAL_COLUMN_RADIUS, CENTRAL_COLUMN_RADIUS, STAIRCASE_HEIGHT, 16);
        const columnMaterial = new THREE.MeshStandardMaterial({ color: 0x8B4513, roughness: 0.7, metalness: 0.3 }); // Brown wood-like
        const centralColumn = new THREE.Mesh(columnGeometry, columnMaterial);
        centralColumn.position.set(STAIRCASE_CENTER_X, STAIRCASE_BASE_Y + STAIRCASE_HEIGHT / 2, STAIRCASE_CENTER_Z);
        scene.add(centralColumn);
        
        // Spiral steps
        const stepMaterial = new THREE.MeshStandardMaterial({ color: 0xD2B48C, roughness: 0.6, metalness: 0.2 }); // Tan wood-like
        
        for (let i = 0; i < STEP_COUNT; i++) {
            const angle = i * STEP_ANGLE_INCREMENT;
            const stepY = STAIRCASE_BASE_Y + i * STEP_HEIGHT;
            
            // Step geometry (wedge-shaped)
            const stepGeometry = new THREE.BoxGeometry(STAIRCASE_RADIUS - CENTRAL_COLUMN_RADIUS, STEP_HEIGHT * 0.8, 0.3);
            const step = new THREE.Mesh(stepGeometry, stepMaterial);
            
            // Position and rotate the step
            const stepDistanceFromCenter = CENTRAL_COLUMN_RADIUS + (STAIRCASE_RADIUS - CENTRAL_COLUMN_RADIUS) / 2;
            step.position.x = STAIRCASE_CENTER_X + Math.cos(angle) * stepDistanceFromCenter;
            step.position.y = stepY;
            step.position.z = STAIRCASE_CENTER_Z + Math.sin(angle) * stepDistanceFromCenter;
            
            // Rotate step to follow the spiral
            step.rotation.y = angle;
            
            scene.add(step);
            
            // Add handrail supports
            const supportGeometry = new THREE.CylinderGeometry(0.05, 0.05, STEP_HEIGHT, 8);
            const supportMaterial = new THREE.MeshStandardMaterial({ color: 0x654321, roughness: 0.5, metalness: 0.5 }); // Dark brown
            const support = new THREE.Mesh(supportGeometry, supportMaterial);
            support.position.x = STAIRCASE_CENTER_X + Math.cos(angle) * (STAIRCASE_RADIUS + 0.1);
            support.position.y = stepY + STEP_HEIGHT / 2;
            support.position.z = STAIRCASE_CENTER_Z + Math.sin(angle) * (STAIRCASE_RADIUS + 0.1);
            scene.add(support);
        }
        ...
        for (let i = 0; i <= STEP_COUNT; i++) {
            const angle = i * STEP_ANGLE_INCREMENT;
            const handrailY = STAIRCASE_BASE_Y + i * STEP_HEIGHT + STEP_HEIGHT * 0.8;
            handrailPoints.push(
                new THREE.Vector3(
                    STAIRCASE_CENTER_X + Math.cos(angle) * (STAIRCASE_RADIUS + 0.1),
                    handrailY,
                    STAIRCASE_CENTER_Z + Math.sin(angle) * (STAIRCASE_RADIUS + 0.1)
                )
            );
        }
        
        const handrailGeometry = new THREE.TubeGeometry(
        ...
        const handrailMaterial = new THREE.MeshStandardMaterial({ color: 0x8B4513, roughness: 0.6, metalness: 0.3 });
