...
const DesktopGallery: React.FC<DesktopGalleryProps> = ({ setInstructionsVisible }) => {
    ...
    async (url: string, panel: Panel, contentType: string): Promise<THREE.Texture | THREE.VideoTexture> => {
      ...
      if (isVideo) {
        return new Promise((resolve) => {
          ...
          const controlsActive = (window as any).galleryControls?.isLocked?.() ?? false;
          if (controlsActive) {
  ...
  const performTeleport = useCallback((targetY: number) => {
    ...
    setTimeout(() => {
      ...
      // Relock controls after fade
      controlsRef.current.lock();
      // Note: isTeleportingRef is reset in the animation loop after fade out
    }, FADE_DURATION * 1000);
  }, []);

  // Interaction handler for desktop (center reticle click)
  const handleInteraction = useCallback((event: MouseEvent | TouchEvent) => {
    if (!controlsRef.current?.isLocked) return; // Only interact if locked on desktop

    // Desktop raycasting is always from the center (0, 0)
  ...
  useEffect(() => {
    ...
    // Add click listener to the renderer element to trigger interaction when locked
    ...
    (window as any).galleryControls = {
      ...
      isLocked: () => controls?.isLocked ?? false,
    ...
    const onContextLost = (event: WebGLContextEvent) => {
      ...
      console.warn('[DesktopGallery] WebGL context lost');
    };
    const onContextRestored = () => {
      console.info('[DesktopGallery] WebGL context restored');
    };
    ...
    const animate = () => {
      ...
      if (controlsRef.current?.isLocked && cameraRef.current && raycasterRef.current) {
        const cam = cameraRef.current;
        const raycaster = raycasterRef.current;
  ...
  }, [setInstructionsVisible, updatePanelContent, manageVideoPlayback, handleInteraction, performTeleport]);
