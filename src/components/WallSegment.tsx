...
import type { AnimatedTexture } from '@/utils/textureLoader'; // Import AnimatedTexture type
...
export type PanelHandles = {
  ...
  animatedTexture?: AnimatedTexture; // New field to hold GIF/Canvas texture handle
  updateContent: (metadata: NftMetadata, texture: THREE.Texture) => void; // Updated signature
  dispose: () => void;
};
...
export class WallSegment {
  ...
  private createPanel(desc: PanelDescriptor, wallMesh: THREE.Mesh): PanelHandles {
    ...
    // New dimensions for text panels (1.5x larger than 2.25)
    const TEXT_PANEL_WIDTH = 3.375; 
    const TEXT_PANEL_HEIGHT = 1.8;
    const TEXT_FONT_SIZE_DESC = 28;
    const TEXT_FONT_SIZE_ATTR = 30; 
    
    // New offsets to keep the panels within the wall boundaries (10 units wide, edge at +/- 5.0)
    // New center offset: 4.5 (max edge) - 3.375/2 (half width) = 2.8125
    const TEXT_BLOCK_OFFSET_X_LEFT = -2.8125; 
    const TEXT_BLOCK_OFFSET_X_RIGHT = 2.8125; 
    ...
    const panelHandle: PanelHandles = {
      ...
      updateContent: (metadata: NftMetadata, texture: THREE.Texture) => {
        try {
          // Update NFT image/video texture
          ...
          const nameTex = createTextTexture(nftNameText, TITLE_NAME_WIDTH, TITLE_NAME_HEIGHT, TITLE_NAME_FONT_SIZE).texture; 
  ...
  public setPanelMetadataById(id: string, metadata: NftMetadata, texture: THREE.Texture) {
    const panel = this.panels.find(p => p.id === id);
    if (!panel) return;
    panel.updateContent(metadata, texture);
  }
