...
const NftGalleryMobile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Structural elements (Same as desktop)
    ...
    const platform = new THREE.Mesh(
      new THREE.BoxGeometry(30, WALL_THICKNESS, 30),
      wallMaterial.clone()
    );
    platform.position.set(0, PLATFORM_Y, 0);
    scene.add(platform);

    const underPlatform = new THREE.Mesh(
    ...
    const buttonGeo = new THREE.CylinderGeometry(1.0, 1.0, TELEPORT_BUTTON_HEIGHT, 32);
    ...
    const groundButton = new THREE.Mesh(buttonGeo, buttonMat.clone());
    groundButton.position.set(0, 0.1 + TELEPORT_BUTTON_HEIGHT / 2, 0);
    groundButton.userData = { isTeleportButton: true, targetY: PLATFORM_Y + 1.6 + WALL_THICKNESS / 2 };
    scene.add(groundButton);

    const upperButton = new THREE.Mesh(buttonGeo, buttonMat.clone());
    upperButton.position.set(0, PLATFORM_Y + WALL_THICKNESS / 2 + TELEPORT_BUTTON_HEIGHT / 2, 0);
    upperButton.userData = { isTeleportButton: true, targetY: 1.6 };
    scene.add(upperButton);
    teleportButtonsRef.current = [groundButton, upperButton];
