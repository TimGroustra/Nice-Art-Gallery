...
const NftGalleryMobile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const CROSS_WALL_BOUNDARY = 5; const crossWallSegments = [-10, 10];
    crossWallSegments.forEach((segmentCenter) => {
      const w1 = new THREE.Mesh(crossWallGeometry, wallMaterial.clone()); w1.position.set(segmentCenter, LOWER_WALL_HEIGHT / 2, -CROSS_WALL_BOUNDARY); scene.add(w1);
      const w2 = new THREE.Mesh(crossWallGeometry, wallMaterial.clone()); w2.position.set(segmentCenter, LOWER_WALL_HEIGHT / 2, CROSS_WALL_BOUNDARY); scene.add(w2);
      const w3 = new THREE.Mesh(crossWallGeometry, wallMaterial.clone()); w3.rotation.y = Math.PI / 2; w3.position.set(-CROSS_WALL_BOUNDARY, LOWER_WALL_HEIGHT / 2, segmentCenter); scene.add(w3);
      const w4 = new THREE.Mesh(crossWallGeometry, wallMaterial.clone()); w4.rotation.y = Math.PI / 2; w4.position.set(CROSS_WALL_BOUNDARY, LOWER_WALL_HEIGHT / 2, segmentCenter); scene.add(w4);
    });
    ...
    const buttonMat = new THREE.MeshStandardMaterial({ color: TELEPORT_BUTTON_COLOR, emissive: TELEPORT_BUTTON_COLOR, emissiveIntensity: 0.5, roughness: 0.1, metalness: 0.9 });
    ...
    const uBtn = new THREE.Mesh(buttonGeo, buttonMat.clone()); uBtn.position.set(0, PLATFORM_Y + WALL_THICKNESS / 2 + 0.1, 0); uBtn.userData = { isTeleportButton: true, targetY: 1.6 }; scene.add(uBtn);
    ...
    const createPanels = async () => {
      ...
      const arrowGeo = new THREE.ShapeGeometry(arrowShape); const ARROW_DEPTH_OFFSET = 0.15 + WALL_THICKNESS / 2; const ARROW_PANEL_OFFSET = 3.2;
      ...
      for (let i = 0; i <= 4; i++) {
        for (const wallNameBase of WALL_NAMES) {
          ...
          const tiers: { y: number; suffix: '-ground' | '-first' }[] = [{ y: LOWER_PANEL_Y, suffix: '-ground' }, { y: UPPER_PANEL_Y, suffix: '-first' }];
          for (const tier of tiers) {
            ...
            const wallRotation = new THREE.Euler(0, rotY, 0, 'XYZ'); const rightVector = new THREE.Vector3(1, 0, 0).applyEuler(wallRotation);
            const prevArrow = new THREE.Mesh(arrowGeo, new THREE.MeshBasicMaterial({ color: 0xcccccc, side: THREE.DoubleSide })); prevArrow.rotation.y = rotY + Math.PI; prevArrow.position.copy(mesh.position).addScaledVector(rightVector, -ARROW_PANEL_OFFSET); scene.add(prevArrow);
            const nextArrow = new THREE.Mesh(arrowGeo, new THREE.MeshBasicMaterial({ color: 0xcccccc, side: THREE.DoubleSide })); nextArrow.rotation.y = rotY; nextArrow.position.copy(mesh.position).addScaledVector(rightVector, ARROW_PANEL_OFFSET); scene.add(nextArrow);
            ...
            tempPanels.push(p); panelsRef.current.push(p);
          }
        }
      }
      crossWallSegments.forEach((sc, idx) => {
        ...
        configs.forEach(cfg => {
          ...
          const wallRotation = new THREE.Euler(0, cfg.rot, 0, 'XYZ'); const rightVector = new THREE.Vector3(1, 0, 0).applyEuler(wallRotation);
          const prevArrow = new THREE.Mesh(arrowGeo, new THREE.MeshBasicMaterial({ color: 0xcccccc, side: THREE.DoubleSide })); prevArrow.rotation.y = cfg.rot + Math.PI; prevArrow.position.copy(mesh.position).addScaledVector(rightVector, -ARROW_PANEL_OFFSET); scene.add(prevArrow);
          const nextArrow = new THREE.Mesh(arrowGeo, new THREE.MeshBasicMaterial({ color: 0xcccccc, side: THREE.DoubleSide })); nextArrow.rotation.y = cfg.rot; nextArrow.position.copy(mesh.position).addScaledVector(rightVector, ARROW_PANEL_OFFSET); scene.add(nextArrow);
      ...
      for (let i = 0; i < tempPanels.length; i++) { if (stopLoad) break; const p = tempPanels[i]; updatePanelContent(p, getCurrentNftSource(p.wallName)); if (i % 3 === 0) await new Promise(resolve => setTimeout(resolve, 100)); }
    };
    createPanels();
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      rotationRef.current.pitch = Math.max(-Math.PI / 2.2, Math.min(Math.PI / 2.2, rotationRef.current.pitch)); touchStartRef.current = { x: e.touches[0].clientX, y: e.touches[0].clientY };
    };
    const handleTouchEnd = (e: TouchEvent) => {
      if (!isDraggingRef.current) {
        ...
        raycasterRef.current.setFromCamera(new THREE.Vector2(x, y), camera); const objects = [...panelsRef.current.flatMap(p => [p.mesh, p.prevArrow, p.nextArrow]), ...teleportButtonsRef.current]; const intersects = raycasterRef.current.intersectObjects(objects);
    ...
    const animate = () => {
      const time = performance.now(); const delta = (time - lastTime) * 0.001; lastTime = time; rainbowMaterial.uniforms.time.value = time * 0.001;
      if (camera) {
        camera.rotation.set(rotationRef.current.pitch, rotationRef.current.yaw, 0);
        if (isWalkingRef.current && !isTeleportingRef.current) {
          const moveSpeed = 4.0; const forward = new THREE.Vector3(0, 0, -1).applyQuaternion(camera.quaternion); forward.y = 0; forward.normalize();
          const nextX = new THREE.Vector3(camera.position.x + forward.x * moveSpeed * delta, camera.position.y, camera.position.z); if (!checkCollision(nextX)) camera.position.x = nextX.x;
          const nextZ = new THREE.Vector3(camera.position.x, camera.position.y, camera.position.z + forward.z * moveSpeed * delta); if (!checkCollision(nextZ)) camera.position.z = nextZ.z;
        }
      ...
      if (isTeleportingRef.current && fadeMaterialRef.current) {
        ...
        if (elapsed < half) fadeMaterialRef.current.opacity = elapsed / half; else if (elapsed < 2 * half) fadeMaterialRef.current.opacity = 1 - (elapsed - half) / half; else { fadeMaterialRef.current.opacity = 0; isTeleportingRef.current = false; }
      }
